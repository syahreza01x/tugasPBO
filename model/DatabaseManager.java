...
public class DatabaseManager {
    ...
    public void initDatabase() {
        try (Statement st = conn.createStatement()) {
            st.executeUpdate("CREATE DATABASE IF NOT EXISTS game_db DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci");
            st.executeUpdate("USE game_db");

            st.executeUpdate("CREATE TABLE IF NOT EXISTS skills (" +
            ...
            st.executeUpdate("CREATE TABLE IF NOT EXISTS powerup (" +
                    "id INT NOT NULL AUTO_INCREMENT PRIMARY KEY," +
            ...
            if (rs.next() && rs.getInt(1) == 0) {
                st.executeUpdate("INSERT INTO skills (id, nama_skill, penjelasan, cooldown, sounds) VALUES" +
                        ...
                        "(7, 'Silver Chariot (Requiem)', 'Memanggil makhluk summon yang otomatis mengejar lawan selama 10 detik', 120000, 'skill7.wav')," +
                        "(8, 'Gold Experience (Requiem)', 'Mengaktifkan time loop: lawan menerima damage terus menerus selama 10 detik dan HP tersisa 1', 120000, 'skill8.wav')");
            }

            rs = st.executeQuery("SELECT COUNT(*) FROM powerup");
            if (rs.next() && rs.getInt(1) == 0) {
                st.executeUpdate("INSERT INTO powerup (id, namapowerup, keterangan) VALUES" +
